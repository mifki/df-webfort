...
function onMessage(evt) {
	...
	if (data[0] == 110) {
		...
		for (var k = 4; k < data.length; k += 5) {
			...
			ctx.drawImage(cd, ((bg % 4) * 256) + 15 * 16, (Math.floor(bg / 4) * 256) +
				15 * 16, 16, 16, x * 16, y * 16, 16, 16);
	...
	function colorize(img, cnv) {
		...
		for (var j = 0; j < 4; j++) {
			for (var i = 0; i < 4; i++) {
				...
				ctx3.fillStyle = 'rgb(' + colors[c * 3 + 0] + ',' + colors[c * 3 + 1] +
					',' + colors[c * 3 + 2] + ')';
...
document.onkeydown = function(ev) {
	...
	if (ev.keyCode == 91 || ev.keyCode == 18 || ev.keyCode == 17 || ev.keyCode ==
		16)
		return;
...
function resizeCanvas() {
	if (active) {
		...
		var data = new Uint8Array([112, Math.floor(canvas.width / 16), Math.floor(
			canvas.height / 16)]);
		websocket.send(data);
	}
}
